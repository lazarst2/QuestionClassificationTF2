...
from bert.tools.TopicsGraph import TopicsGraph
  ...
  def _read_json(cls, input_file, quotechar=None):
    ...
    with tf.io.gfile.GFile(input_file, "r") as f:
      ...
      for line in f:
        ...
        lines.append([data["title"], data["tags"]])
      return lines

class QuestionClassificationProcessor(DataProcessor):
  def __init__(self,data_dir):
    self.data_dir = data_dir
    graphFile = os.path.join(self.data_dir,"topics.json")
    self.graph = TopicsGraph.parseJsonAndCreateGraph(graphFile)
      
      
      
  def get_train_examples(self, data_dir):
    """See base class."""
    # if(FLAGS.use_full_trainset):
    #   train_filename = "train_full.tsv"
    # else:
    #   train_filename = "train.tsv"

    train_filename = "train.json"
    return self._create_examples(
        self._read_json(os.path.join(data_dir, train_filename)), "train")
  ...
  def get_labels(self):
    """See base class."""
    return self.graph.get_list_nodes()
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      # print("{\"tags\":"+line[1]+"}")
      label = line[1]
      #print(line[1],ast.literal_eval(line[1]))
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if ex_index < 5:
    ...
    logging.info("label: %s (id = %d)", example.label, one_hot_labels)
