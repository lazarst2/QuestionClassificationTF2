...
from bert import model_training_utils
from bert import modeling
from bert import optimization
from bert import tpu_lib
from bert import bert_models_library as bert_lib
from bert import metrics
...
def run_customized_training(strategy,
  ...
  def _get_classifier_model():
    ...
    #     bert_lib.BertMultiLabelClassifier.get(bert_config, tf.float32, num_classes,
    #                                  max_seq_length))

    classifier_model, core_model = bert_models.classifier_model(bert_config, tf.float32, num_classes, max_seq_length)
  ...
  def get_loss_fn():
    ...
    def classification_loss_fn(labels, logits):
      ...
      #print(labels,logits)
      one_hot_labels = tf.cast(labels, dtype=tf.float32)
  ...
  def metric_fn():
    return metrics.F1_Score(dtype=tf.float32)
...
def export_classifier(model_export_path, input_meta_data):
  ...
  def _model_fn():
    return  bert_lib.BertMultiLabelClassifier.get(bert_config, tf.float16,
